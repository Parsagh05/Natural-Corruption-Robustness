...
def clipped_zoom(img, zoom_factor):
    h, w = img.shape[:2]
    # Crop and trim height/width independently so rectangular images keep
    ...
    img = scizoom(
        img[top:top + ch, left:left + cw], zoom_tuple, order=1
    )
    # Trim any extra pixels introduced by interpolation rounding.
...
def impulse_noise(x, severity=1):
    ...
    # random_noise(rng=None) creates an independent Generator and ignores the
    # fixed NumPy state used by the harness. Derive its Generator from that
    # state so per-image corruption seeding is actually reproducible.
    rng = np.random.default_rng(np.random.randint(0, 2**32, dtype=np.uint32))
    x = sk.util.random_noise(
        np.array(x) / 255., mode='s&p', amount=c, rng=rng
    )
